...
def process_packet(packet):
    if IP in packet:
        ...
        is_blacklisted = any(is_ip_in_range(ip_src, r) for r in blacklist_ranges)
        is_whitelisted = any(is_ip_in_range(ip_src, r) for r in whitelist_ranges)

        if is_blacklisted:
            console.print(f"[red]🔴 Blacklisted IP Detected: {ip_src}[/red]")
            log_to_csv(ip_src, "Blacklisted")
        elif is_whitelisted:
            console.print(f"[green]🟢 Whitelisted IP: {ip_src}[/green]")
            log_to_csv(ip_src, "Whitelisted")
        else:
            console.print(f"[yellow]🟡 Unrecognized IP: {ip_src}[/yellow]")
            log_to_csv(ip_src, "Unrecognized")

        update_dashboard(ip_src, is_blacklisted, is_whitelisted)
...
def send_professional_email(subject, attachment_path, ip_count, time_range):
        ...
        <div style="max-width: 600px; margin: auto; border: 1px solid #ddd; padding: 20px; border-radius: 8px;">
            ...
            <div style="margin-top: 40px; border-top: 1px solid #eee; padding-top: 20px;">
                <p style="font-size: 12px; color: #666;">
                    ...
                    <a href="https://firewallnotifier.cyberfascinate.com" style="1
                    ...
                    ">Visit Website</a>
                    <br><br>
                    Maintained by <a href='mailto:contact@cyberfascinate.com' style='color: #007BFF; text-decoration: none;'>Kashish Kanojia</a><br>
    ...
    try:
        ...
        print(f"[+] Sent professional report: {subject}")
    except Exception as e:
...
# CLI Banner
def show_banner():
    title = Text("Firewall Notifier", style="bold blue")
    subtitle = Text("Simple. Secure. Smarter Defense.", style="italic green")
    maintainer = Text("by CyberFascinate", style="cyan")

    panel = Panel(
        f"{title}\n{subtitle}\n\n{maintainer}\nMIT Licensed • Self-Hosted & Secure",
